...
class TestMiniMaxCLIOptions:
    ...
    def test_minimax_emotion_option(self):
        """测试 MiniMax 情感选项"""
        with patch('sys.argv', ['vidppt', 'test.pptx', '--tts-engine', 'minimax', '--minimax-emotion', 'happy']):
            with patch('vidppt.cli.Pipeline') as mock_pipeline:
                try:
                    main()
                except SystemExit:
    ...
    def test_minimax_sample_rate_option(self):
        """测试 MiniMax 采样率选项"""
        with patch('sys.argv', ['vidppt', 'test.pptx', '--tts-engine', 'minimax', '--minimax-sample-rate', '44100']):
            with patch('vidppt.cli.Pipeline') as mock_pipeline:
                try:
                    main()
                except SystemExit:
    ...
    def test_minimax_bitrate_option(self):
        """测试 MiniMax 比特率选项"""
        with patch('sys.argv', ['vidppt', 'test.pptx', '--tts-engine', 'minimax', '--minimax-bitrate', '256000']):
            with patch('vidppt.cli.Pipeline') as mock_pipeline:
                try:
                    main()
                except SystemExit:
    ...
    def test_minimax_format_option(self):
        """测试 MiniMax 格式选项"""
        with patch('sys.argv', ['vidppt', 'test.pptx', '--tts-engine', 'minimax', '--minimax-format', 'wav']):
            with patch('vidppt.cli.Pipeline') as mock_pipeline:
                try:
                    main()
                except SystemExit:
...
class TestEdgeTTSCLIOptions:
    ...
    def test_voice_option(self):
        """测试声音选项"""
        with patch('sys.argv', ['vidppt', 'test.pptx', '--voice', 'zh-CN-YunyangNeural']):
            with patch('vidppt.cli.Pipeline') as mock_pipeline:
                try:
                    main()
                except SystemExit:
...
class TestCLIEdgeCases:
    ...
    def test_feature_flags_combination(self):
        """测试功能标志组合"""
        with patch('sys.argv', ['vidppt', 'test.pptx', '--no-tts', '--no-video', '--no-intermediate']):
            with patch('vidppt.cli.Pipeline') as mock_pipeline:
                try:
                    main()
                except SystemExit:

...
Web 界面内置可独立开关的系统监控仪表盘，实时显示 CPU 利用率和内存使用量。
...
启动后页面右下角会出现显示器图标按钮，点击即可展开/折叠仪表盘卡片。仪表盘每 3 秒自动刷新数据，环形图带有平滑动画效果。

**关闭仪表盘：**

- 关闭后端：设置环境变量 `VIDPPT_DASHBOARD=false`，API 端点将不再注册，前端检测到 404 后显示"不可用"状态
- 关闭前端：移除 `index.html` 中 `dashboard.js` 的引用，页面无任何仪表盘痕迹
- 完全移除：删除 `web/api/system_stats.py`、`web/static/css/dashboard.css`、`web/static/js/dashboard.js` + `index.html` 中 5 行引用 + `app.py` 中 3 行注册代码
